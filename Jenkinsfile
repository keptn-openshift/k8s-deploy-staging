...
pipeline {
  ...
  stages {
    stage('Update Deployment and Service specification') {
      steps {
        container('git') {
          sshagent (credentials: ['git-ssh-credentials-acm'])  {
            sh "git config --global user.email ${env.GITHUB_USER_EMAIL}"
            sh "git clone git@github.com/${env.GITHUB_ORGANIZATION}/k8s-deploy-staging"
            ...
            sh "cd k8s-deploy-staging/ && git push git@github.com/${env.GITHUB_ORGANIZATION}/k8s-deploy-staging"
          }
        }
      }
    }
